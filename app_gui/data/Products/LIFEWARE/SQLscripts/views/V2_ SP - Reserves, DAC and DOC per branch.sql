DROP VIEW IF EXISTS `V2: SP - Reserves, DAC and DOC per branch`;
CREATE VIEW `V2: SP - Reserves, DAC and DOC per branch` AS
    SELECT
    ...
    WHERE Bestandsreport.`Regular Premium (EUR)` IS NULL
