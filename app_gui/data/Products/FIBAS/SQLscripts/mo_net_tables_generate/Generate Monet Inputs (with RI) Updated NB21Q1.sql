DROP TABLE IF EXISTS MonetInputsUpdated_NB21Q1;
CREATE TABLE IF NOT EXISTS MonetInputsUpdated_NB21Q1 AS
...
IF(LOCATE(`benefit_duration`,"12 months")>0 Or LOCATE(`benefit_duration`,"24 months")>0,"Short",
    IF(LOCATE(`benefit_duration`,"36 months")>0 Or LOCATE(`benefit_duration`,"48 months")>0 Or LOCATE(`benefit_duration`,"60 months")>0 Or LOCATE(`benefit_duration`,"72 months")>0 Or LOCATE(`benefit_duration`,"84 months")>0,"Basic",IF(LOCATE(`benefit_duration`,"96 months")>0 Or LOCATE(`benefit_duration`,"108 months")>0 Or LOCATE(`benefit_duration`,"120 months")>0 Or `benefit_duration`="Enddate","Extended",`benefit_duration`))) AS `BeneDuration`,
...
FROM `Policies` LEFT JOIN `MonetInputsClaimsUpdated` ON `Policies`.`policy number` = `MonetInputsClaimsUpdated`.`PolNo`
    WHERE (((`Policies`.`Quantum status`)="Active")
               ...
               And (Year(STR_TO_DATE(`Policies`.`commencement date`, '%d-%m-%Y'))=2021)
               And (Month(STR_TO_DATE(`Policies`.`commencement date`, '%d-%m-%Y'))=1
                        Or (Month(STR_TO_DATE(`Policies`.`commencement date`, '%d-%m-%Y')))=2
                        Or (Month(STR_TO_DATE(`Policies`.`commencement date`, '%d-%m-%Y')))=3))
