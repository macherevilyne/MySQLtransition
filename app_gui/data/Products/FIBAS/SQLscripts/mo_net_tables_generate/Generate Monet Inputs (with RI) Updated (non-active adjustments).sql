...
IF(`policies`.`product`="TAF Maandlastbeschermer Zelfstandige","ZSP",`policies`.`product group`) AS `product`,
                ...
                IF(`Policies`.`product` Like "%2011",2011,
                    IF(`policies`.`policy terms`="QL_MLB_12_2012",2011,
                        IF(`Policies`.`policy terms`="QL_MLBK_03_2014" Or `policies`.`policy terms`="QL_MLBK_11_2015",2014,
                            IF(`Policies`.`policy terms`="QL_MLB_06_2019" Or `policies`.`policy terms`="QL_MLB_2021_02" Or `policies`.`policy terms`="QL_MLB_2021_11",2019,
                                IF(`policies`.`policy terms`="QL_MLBZ_2022_02",2020,200809))))))) AS `book`,
...
IF((SELECT `book`)=200809 And IF(`policies`.`product`="TAF Maandlastbeschermer Zelfstandige","ZSP",`policies`.`product group`)="ZSP" And (ISNULL(`monetInputsClaimsUpdated`.`status`) Or Not IF(ISNULL(`MonetInputsClaimsUpdated`.`PolNo`),"active","disable")="disable" Or `MonetInputsClaimsUpdated`.`PeriodDisable`<TIMESTAMPDIFF(MONTH,STR_TO_DATE('01.09.2011', '%d.%m.%Y'),STR_TO_DATE('{ValDat}', '%d-%m-%Y'))),12,
    IF((SELECT `book`)=2012 Or (SELECT `book`)=2015 Or (SELECT `book`)=2020,12,24)) AS MthOwnOccupation,
        IF((SELECT `book`)=2010 Or (IF(`policies`.`product`="TAF Maandlastbeschermer Zelfstandige","ZSP",`policies`.`product group`)="MLB" And ((SELECT `book`)<2019 Or (SELECT `book`)=200809)) Or (IF(ISNULL(`MonetInputsClaimsUpdated`.`PolNo`),"active","disable")="disable" And `PeriodDisable`>=TIMESTAMPDIFF(MONTH,STR_TO_DATE('01.09.2011', '%d.%m.%Y'),STR_TO_DATE('{ValDat}', '%d-%m-%Y'))),"Occupation","Immediate") AS `MethPartial`,
`Policies`.`mental diseases`,
IF((SELECT `book`)=200809,
    IF(IF(`policies`.`product`="TAF Maandlastbeschermer Zelfstandige","ZSP",`policies`.`product group`)="MLB","Yes","Restricted"),
