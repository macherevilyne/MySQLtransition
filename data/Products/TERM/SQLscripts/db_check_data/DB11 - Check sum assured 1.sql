...
SELECT
    ...
    IF(
        Policydata_to_Quantum_ORIGINAL.`Insured life 1 SA` IS NULL,
        'Sum assured 1 empty',
        'Invalid sum assured 1'
    ) AS Error,
    'Insured life 1 SA' AS Variable,
    Policydata_to_Quantum_ORIGINAL.`Insured life 1 SA` AS `Value`,
    IF(
...
WHERE
    ...
    OR (
        (Policydata_to_Quantum_ORIGINAL.`Insured life 1 SA` < 0)
        AND (
            IF(
                Policydata_to_Quantum_ORIGINAL.`Internal status` IS NULL,
                'Undefined',
