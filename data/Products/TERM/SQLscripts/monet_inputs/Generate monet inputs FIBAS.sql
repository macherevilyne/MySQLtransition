...
SELECT
    ...
    TIMESTAMPDIFF(MONTH,STR_TO_DATE(`commencement date`, '%d-%m-%Y'),STR_TO_DATE('{date_data_extract}', '01-%m-%Y')) AS PeriodIF,
    "Term" AS Product,
    "No" AS Joined,
    sex AS Sex1,
    sex AS Sex2,
...
WHERE
    (
        ...
        CASE
            WHEN ISNULL(`benefit_duration_term_life`) OR `benefit_duration_term_life` = "Standard" THEN 0
            WHEN CalcBeneDur(STR_TO_DATE(`benefit_duration_term_life`, '%d-%m-%Y')) >=
                CASE
                    WHEN `Premium payment` IN ("Single premium", "Combination postponed") THEN `SP term`
                    ELSE `RP term`
                END - 6
    ...
    AND STR_TO_DATE(`commencement date`, '%d-%m-%Y') < STR_TO_DATE('{date_data_extract}', '01-%m-%Y')
