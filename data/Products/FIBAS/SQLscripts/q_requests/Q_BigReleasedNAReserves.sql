DROP VIEW IF EXISTS `Q_BigReleasedNAReserves`;
CREATE VIEW `Q_BigReleasedNAReserves` AS
SELECT 
...
GROUP BY 
    ...
    `MonetResultsNotAcceptPrevious`.`Status`
HAVING 
    (((`MonetResultsNotAccept`.`Status`)="active")
    AND ((`MonetResultsNotAcceptPrevious`.`Status`)="disable")) 
    OR (((`MonetResultsNotAccept`.`PolNo`) Is Null)
    AND ((`MonetResultsNotAcceptPrevious`.`Status`)="disable"))
ORDER BY 
